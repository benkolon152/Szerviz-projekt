...
function buildInsertQuery(rows) {
  const staticColumns = ["category", "name", "sort_order", "price_huf", "specifications"];
  const allColumns = [...staticColumns, ...FLAT_SPEC_COLUMNS];
  const values = [];
  const placeholders = rows
    .map(([category, name, sortOrder, priceHuf, specifications], index) => {
      const normalized = normalizeSpecifications(specifications);
      const rowValues = [category, name, sortOrder, priceHuf, JSON.stringify(specifications)];
...
async function main() {
  ...
  await pool.query("ALTER TABLE pc_components ADD COLUMN IF NOT EXISTS price_huf INTEGER NOT NULL DEFAULT 0");
  await pool.query("ALTER TABLE pc_components ADD COLUMN IF NOT EXISTS specifications JSONB NOT NULL DEFAULT '{}'::jsonb");
  await pool.query(`
  ...
  await pool.query(`TRUNCATE TABLE pc_components`);
